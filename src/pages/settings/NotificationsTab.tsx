...
const NotificationsTab = () => {
  ...
  // Fetch patches when daysAhead changes
  useEffect(() => {
    const fetchPatches = async () => {
      ...
      try {
        ...
        setNotification({
          type: 'error',
          message: 'Failed to load upcoming patches',
        });
      } finally {
        setIsLoading(false);
      }
  ...
  const handleSendNow = async () => {
    if (patches.length === 0) {
      setNotification({
        type: 'error',
        message: 'No upcoming patches to notify about',
      });
      return;
    }
    setShowConfirm(true);
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      setNotification({
        type: 'success',
        message: `Notification sent for ${result.upcomingPatchesCount} upcoming patches`,
      });
    } catch (error) {
    ...
    <div className="bg-white rounded-lg border border-gray-200 p-6">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-600">
          Patch reminders sent daily at 9:00 AM to the configured Slack
          channel. Showing patches due within{' '}
          <select
            value={daysAhead}
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-200">
              {patches.map((patch) => {
                ...
                const date = new Date(patch.nextScheduledPatchDate);
                ...
                return (
                  <tr key={patch.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`inline-block px-2.5 py-1 rounded-full text-xs font-medium ${urgency.className}`}
                      >
      ...
      <ConfirmDialog
        ...
        message={`Send patch reminders for ${patches.length} upcoming patches?`}
        confirmLabel="Send"
        variant="warning"
        isLoading={isSending}
      />
